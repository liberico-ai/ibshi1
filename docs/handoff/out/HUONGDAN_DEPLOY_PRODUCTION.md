**IBS HEAVY INDUSTRY — HỆ THỐNG ERP**
...
## 1. Tổng quan đợt (PR #48 → #76 · 14/07 → 22/07/2026)
...
| **#68** | Áp template GIỮA flow (`fromStepCode`) — dự án đã tiến xa không nhận task lệch; tổ tiên gồm cả `gateCodes` | code-only |
| — | Người giao **sửa danh sách người nhận** (chỉ người CHƯA hoàn thành) + **recall/Xóa-Sửa** qua QTHT (chỉ R10) | code-only |
| — | **Batch nền** (sau #67): RBAC động zero-schema (capability×role×user); thông báo trong app + nhắc quá hạn; liên kết Telegram tự phục vụ; export Excel + auto-xóa >90 ngày Nhật ký/Error Logs; guard `/api/cron/*` bằng `CRON_SECRET`; bỏ `@import` Google Fonts (CSP) | **code-only — KHÔNG đụng DB** (dùng bảng/cột đã có: Notification, telegram_chat_id, AuditLog/ErrorLog) |
| **#70–75** | **Revise Flow36** — revise đi hết chuỗi 36 bước theo `revision_round` + human skip (thay cascade phẳng); `SkipReasonModal` (thay `window.prompt`) | **DB migration** (`add_task_revision_fields`) + **flag build-time** `NEXT_PUBLIC_FF_REVISE_FLOW` + code |
| **#76** | Prod-readiness + **guard fork**: endpoint `GET /api/work/revise/eligibility` + disable nút cho dự án legacy + `scripts/revise-readiness-check.ts` (read-only) | code-only *(⚠ **PR OPEN, chưa merge lúc viết** — xem lưu ý cuối mục 1)* |

**Tóm tắt loại:** DB migration = **4** (`pr_source`, `project_documents`, `purchase_contracts`, **`add_task_revision_fields`** — mới đợt này). Data-backfill = 2 (PR materialize #48 đã chạy đợt trước; **F3 Budget #67 chưa chạy**). Flag = **3** (`FF_PR_MATERIALIZE`, `NEXT_PUBLIC_FF_BOM_CASCADE`, **`NEXT_PUBLIC_FF_REVISE_FLOW`** — mới đợt này). Còn lại **code-only, KHÔNG migration** (xác nhận: từ 17/07 chỉ có **1** migration mới = `add_task_revision_fields`).

> **⚠ #76 (guard) chưa vào main lúc viết doc.** #71–75 (Revise) đã merged. **Nếu bật `NEXT_PUBLIC_FF_REVISE_FLOW=true` trên prod mà CHƯA merge #76** → user chọn dự án **legacy** sẽ thấy lỗi kỹ thuật *"không xác định được template của dự án"* thay vì cảnh báo thân thiện. → **Merge #76 TRƯỚC khi bật FF revise prod** (hoặc chỉ bật khi mọi dự án đích đều template-driven).

> **Revise Flow36 — điều kiện chạy (QUAN TRỌNG).** Sau khi (a) áp migration `add_task_revision_fields` và (b) build image với `NEXT_PUBLIC_FF_REVISE_FLOW=true`, một dự án **chỉ mở được vòng revise nếu đang chạy theo process-template** (có ≥1 task gắn `template_step_id`). Dự án **legacy** (task `template_step_id = null`) → guard #76 disable nút (nếu đã merge) hoặc báo lỗi (nếu chưa). Trước/sau deploy chạy `scripts/revise-readiness-check.ts` (read-only) để biết phạm vi. FF là **build-time** → bật/tắt = rebuild image (không toggle runtime).
>
> **Số liệu readiness — UAT (đã chạy 22/07):** 25 dự án → **19 ĐỦ ĐIỀU KIỆN** (ACTIVE, template 32 bước, 0 cảnh báo) / **6 LEGACY** (5 ACTIVE + 1 CANCELLED). **PROD: CHƯA CHẠY** — devops chạy readiness trên prod và đính số vào đây trước khi bật FF.
