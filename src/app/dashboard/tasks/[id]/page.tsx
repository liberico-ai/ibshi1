...
export default function TaskDetailPage() {
  ...
  const [siblingFiles, setSiblingFiles] = useState<Record<
    string,
    string
  > | null>(null);
  ...
  const [milestones, setMilestones] = useState<
    { name: string; startDate: string; endDate: string; assigneeId: string }[]
  >([]);
  ...
  >([{ ...emptyBomItem }, { ...emptyBomItem }, { ...emptyBomItem }]);
  const emptyWoItem = {
    costCode: "",
    content: "",
    jobCode: "",
  ...
  const [suppliers, setSuppliers] = useState<SupplierEntry[]>([
    { ...emptySupplier, quotes: [{ ...emptyQuote }] },
    { ...emptySupplier, quotes: [{ ...emptyQuote }] },
    { ...emptySupplier, quotes: [{ ...emptyQuote }] },
  ]);
  ...
  const [paymentMilestones, setPaymentMilestones] = useState<
    { label: string; percent: string; date: string }[]
  >([{ label: "Đợt 1", percent: "", date: "" }]);
  const [deliveryType, setDeliveryType] = useState<"full" | "batch">("full");
  const [deliveryBatches, setDeliveryBatches] = useState<
    { material: string; qty: string; date: string }[]
  >([{ material: "", qty: "", date: "" }]);
  const [userList, setUserList] = useState<
    { id: string; fullName: string; roleCode: string }[]
  >([]);
  ...
  const [previousStepData, setPreviousStepData] = useState<{
    ...
    woData?: any;
    lsxData?: any;
    departmentEstimates?: any;
    budgetTotal?: any;
  } | null>(null);
  // P1.2A WBS expanded rows
  const [wbsExpandedRows, setWbsExpandedRows] = useState<Set<number>>(
    new Set(),
  );
  ...
  >("pending");
  const [estimateDecision, setEstimateDecision] = useState<
    "pending" | "approved" | "rejected"
  >("pending");
  ...
  const [assigningTask, setAssigningTask] = useState<TaskData | null>(null);
  ...
  async function loadTask() {
    ...
    if (res.ok) {
      ...
      if (res.task.resultData) {
        ...
        if (rd.paymentType)
          setPaymentType(rd.paymentType as "full" | "partial");
        if (rd.paymentMilestones && Array.isArray(rd.paymentMilestones))
          setPaymentMilestones(
            rd.paymentMilestones as typeof paymentMilestones,
          );
        if (rd.deliveryType)
          setDeliveryType(rd.deliveryType as "full" | "batch");
        if (rd.deliveryBatches && Array.isArray(rd.deliveryBatches))
          setDeliveryBatches(rd.deliveryBatches as typeof deliveryBatches);
        // Restore P4.1 payment confirmations
        if (rd.paymentConfirmations && Array.isArray(rd.paymentConfirmations))
          setPaymentConfirmations(
            rd.paymentConfirmations as typeof paymentConfirmations,
          );
        // Restore P4.4 warehouse items
        if (rd.warehouseItems && Array.isArray(rd.warehouseItems))
          setWarehouseItems(rd.warehouseItems as typeof warehouseItems);
      ...
      if (res.previousStepData) {
        ...
        if (
          res.task.stepCode === "P3.5" &&
          !(res.task.resultData as Record<string, unknown> | null)?.suppliers
        ) {
          const prRaw = (res.previousStepData as Record<string, unknown>)
            ?.prItems;
          if (Array.isArray(prRaw) && prRaw.length > 0) {
            const prQuotes = prRaw
              .filter((p: Record<string, string>) => p.name?.trim())
      ...
      if (res.task.stepCode === "P1.1" && res.task.project) {
        ...
        setFormData((prev) => ({
          ...
          contractValue:
            prev.contractValue ||
            (p.contractValue ? String(p.contractValue) : ""),
      ...
      if (res.task.stepCode === "P6.5" && res.previousStepData) {
        ...
        const totalCost = pd.p62Total
          ? `${Number(pd.p62Total).toLocaleString("vi-VN")} đ`
          : "";
        const variance = pd.p62Variance
          ? ` (chênh lệch: ${Number(pd.p62Variance) > 0 ? "+" : ""}${pd.p62Variance}%)`
          : "";
        const profit = pd.p63Profit
          ? `LN: ${Number(pd.p63Profit).toLocaleString("vi-VN")} đ`
          : "";
        const margin = pd.p63Margin ? ` — Biên LN: ${pd.p63Margin}%` : "";
        setFormData((prev) => ({
          ...prev,
          qcDossierStatus: (pd.p61Status as string) || "⏳ Chưa bắt đầu",
          costSettlement: pd.p62Status
            ? `${pd.p62Status}${totalCost ? ` — ${totalCost}${variance}` : ""}`
            : "⏳ Chưa bắt đầu",
          plSummary: pd.p63Status
            ? `${pd.p63Status}${profit ? ` — ${profit}${margin}` : ""}`
            : "⏳ Chưa bắt đầu",
  ...
  function handleFieldChange(key: string, value: string | number) {
    setFormData((prev) => {
      ...
      if (config && config.fields.some((f) => f.key === "totalEstimate")) {
        const currencyKeys = config.fields
          .filter((f) => f.type === "currency")
          .map((f) => f.key);
        const total = currencyKeys.reduce(
          (sum, k) => sum + (Number(next[k]) || 0),
          0,
        );
  ...
  function addMilestone() {
    setMilestones((prev) => [
      ...prev,
      { name: "", startDate: "", endDate: "", assigneeId: "" },
    ]);
  }
  ...
  const exportMilestonesExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `Milestones_${task?.project?.projectCode || "P1.2A"}.xlsx`,
    );
  };
            ...
            if (key && rowData[ci] != null) {
              ...
              if (key === "startDate" || key === "endDate") {
                ...
                if (!isNaN(Number(val)) && Number(val) > 40000) {
                  const date = new Date(
                    Math.round((Number(val) - 25569) * 86400 * 1000),
                  );
        ...
        if (imported.length > 0) {
          setMilestones((prev) => {
            const cleanPrev = prev.filter(
              (r) => r.name.trim() || r.startDate || r.endDate,
            );
  ...
  async function handleSubmit(action: "complete" | "reject") {
    ...
    if (task.stepCode === "P2.1" && action === "complete") {
      const filledBomItems = bomItems.filter(
        (b) => b.name.trim() && b.code.trim(),
      );
      if (filledBomItems.length < 3) {
        setError(
          "Danh sách vật tư phải có tối thiểu 3 mục (đã nhập tên + mã VT)",
        );
        setSubmitting(false);
        return;
      }
    ...
    if (action === "reject") {
      ...
      if (res.success) {
        setSuccessMsg(
          `✅ Đã từ chối. Quay về bước ${res.returnedTo}: ${res.returnedToName || ""}`,
        );
    ...
    if (
      ...
      const ensuredKeys = [
        {
          key: "dt02Items",
          def: [{ maCP: "I", noiDung: "Chi phí vật tư", giaTri: "", tyLe: "" }],
        },
        {
          key: "dt03Items",
          def: [
      ...
      ensuredKeys.forEach(({ key, def }) => {
        const isRendered =
          ...
          (task.stepCode === "P2.1A" && key === "dt07Items") ||
          task.stepCode === "P2.4";
        if (isRendered && !finalData[key]) {
          const defaultArray = Array.isArray(def) ? def : [def]; // TS compat, def is always array here
          finalData[key] = JSON.stringify(
            getInheritedEstRows(key, defaultArray as Record<string, string>[]),
          );
        }
      });
    }
      ...
      body: JSON.stringify({
        ...
        resultData: {
          ...
          ...(bomItems.filter((b) => b.name.trim()).length > 0
            ...
            : {}),
          ...(woItems.filter((w) => w.content.trim()).length > 0
            ? { woItems: woItems.filter((w) => w.content.trim()) }
            : {}),
          ...
          ...(deliveryType === "batch"
            ? {
                deliveryBatches: deliveryBatches.filter((d) =>
                  d.material.trim(),
                ),
              }
            : {}),
    ...
    if (res.ok) {
      setSuccessMsg(
        `✅ Hoàn thành! Bước tiếp: ${res.nextSteps?.join(", ") || "Không có"}`,
      );
      setTimeout(() => router.push("/dashboard/tasks"), 2000);
    } else {
  ...
  const getInheritedEstRows = (key: string, baseDefault: any[]) => {
    if (!task || task.stepCode !== "P2.4" || !previousStepData?.estimate)
      return baseDefault;
    try {
  ...
  const isGlobalAdmin = ["R00", "R01", "R02", "R02a"].includes(
    currentUser?.roleCode || "",
  );
  const baseTaskRole = task.assignedRole.replace(/[a-zA-Z]$/, "");
  const baseUserRole = currentUser?.roleCode?.replace(/[a-zA-Z]$/, "") || "";
  const isAssignedToMe = task.assignee?.id === currentUser?.id;

  // Option C: Admin OR (L1 Manager of same base role IF unassigned) OR (officially assigned to me)
  const isActive =
    isInProgress &&
    !!currentUser &&
    (isGlobalAdmin ||
      ...
      (currentUser.userLevel === 1 &&
        baseUserRole === baseTaskRole &&
        !task.assignee));

  const canAssignLevel = currentUser?.userLevel === 1 || isGlobalAdmin;
  const hasAssignPerm =
    canAssignLevel && (isGlobalAdmin || baseUserRole === baseTaskRole);
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      >
        ...
        >
          <span>
            Phase {rule?.phase}: {phaseName}
          </span>
          <span>•</span>
          <span
            style={{ fontWeight: 600, color: "var(--text-primary, #0f172a)" }}
          >
        ...
        >
          <span
            style={{
              ...
              alignItems: "center",
              gap: 6,
              color: isDone ? "#059669" : isInProgress ? "#2563eb" : "inherit",
            }}
          >
            {isDone
              ? "✅ Hoàn thành"
          ...
          <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span
              className="font-mono text-[10px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800"
            ...
            {hasAssignPerm &&
              ...
              task.status !== "REJECTED" && (
                <button
                  ...
                  className="ml-1 flex items-center justify-center rounded hover:bg-slate-200 dark:hover:bg-slate-700 transition"
      ...
      {isInProgress && !isActive && currentUser && (
        <div
          style={{
            background: "#fef3c7",
        ...
        >
          🔒 Bước này giao cho <strong>{task.assignedRole}</strong> — bạn đang
          xem ở chế độ <strong>chỉ đọc</strong> (role: {currentUser.roleCode})
        </div>
      )}
      ...
      {rejectionInfo &&
        (() => {
          ...
          const title =
            fromStep === "P5.3"
              ? "QC đã từ chối nghiệm thu sản phẩm SX"
              : fromStep === "P4.3"
                ? "QC đã từ chối nghiệm thu nhập kho"
            ...
            >
              ...
              <div style={{ fontSize: "0.8rem", color: "#b91c1c" }}>
                ...
                {rejectionInfo.rejectedAt &&
                  ` — ${new Date(rejectionInfo.rejectedAt).toLocaleString("vi-VN")}`}
              </div>
              ...
              {fromStep === "P5.3" &&
                (() => {
                  const qcItems = (
                    rejectionInfo as {
                      qcItems?: { task: string; result: string }[];
                    }
                  ).qcItems;
                  if (
                    !qcItems ||
                    !Array.isArray(qcItems) ||
                    qcItems.length === 0
                  )
                    return null;
      ...
      {config ? (
          ...
          <div>
            ...
            {task.stepCode === "P1.3" && previousStepData ? (
              <>
                ...
                >
                  ...
                  >
                    ...
                    >
                      📋 Kế hoạch Kickoff / WBS / Milestones{" "}
                      <span
                        style={{
                          fontSize: "0.8rem",
                          color: "var(--text-secondary)",
                    ...
                    {planDecision !== "pending" && (
                      <span
                        style={{
                          ...
                          fontWeight: 700,
                          color:
                            planDecision === "approved" ? "#16a34a" : "#dc2626",
                        }}
                      >
                        {planDecision === "approved"
                          ? "✅ Đã duyệt"
                  ...
                  {previousStepData.plan ? (
                    ...
                    >
                      ...
                      {(() => {
                        ...
                        try {
                          rows =
                            typeof previousStepData.plan.wbsItems === "string"
                              ? JSON.parse(previousStepData.plan.wbsItems)
                              : Array.isArray(previousStepData.plan.wbsItems)
                                ? previousStepData.plan.wbsItems
                                : [];
                        } catch {
                          rows = [];
                        }
                        ...
                        return (
                          <div style={{ gridColumn: "1 / -1", marginTop: 8 }}>
                            ...
                            >
                              Bảng WBS chi tiết ({rows.length} hạng mục)
                            </label>
                            <div
                              style={{
                                overflowX: "auto",
                      ...
                      {previousStepData.plan.milestones &&
                        previousStepData.plan.milestones.length > 0 && (
                          <div style={{ gridColumn: "1 / -1", marginTop: 8 }}>
                            <label
                              style={{
                                fontSize: "0.8rem",
                            ...
                            >
                              Milestones (
                              {previousStepData.plan.milestones.length})
                            </label>
                            <div
                              style={{
                                display: "flex",
                            ...
                            >
                              {previousStepData.plan.milestones.map(
                                (
                                  ms: {
                                    name: string;
                                    startDate: string;
                                  ...
                                  >
                                    ...
                                    >
                                      {ms.startDate &&
                                        `Bắt đầu: ${ms.startDate}`}
                                      {ms.endDate &&
                                        ` — Kết thúc: ${ms.endDate}`}
                                      {ms.assigneeId &&
                                        ` | Phụ trách: ${userList.find((u) => u.id === ms.assigneeId)?.fullName || ms.assigneeId}`}
                                    </div>
                                  </div>
                                ),
                              )}
                  ...
                  {isActive && planDecision === "pending" && (
                    <div style={{ marginTop: 16 }}>
                      <div style={{ display: "flex", gap: 10 }}>
                        <button
                          onClick={async () => {
                            ...
                            try {
                              ...
                              if (res.ok && res.task) {
                                task.resultData = res.task.resultData; // Keep local task in sync
                                setPlanDecision("approved");
                                setSuccessMsg(
                                  "✅ Đã duyệt Kế hoạch (PM). Vui lòng duyệt Dự toán nếu chưa duyệt.",
                                );
                              } else {
                                setError(res.error || "Lỗi lưu thông tin");
                              }
                            } catch {
                              setError("Lỗi khi duyệt kế hoạch");
                            }
                            setSubmitting(false);
                          }}
                          disabled={submitting}
                        ...
                        <button
                          onClick={() => setShowPlanReject(!showPlanReject)}
                          disabled={submitting}
                          style={{
                            padding: "8px 20px",
                      ...
                      {showPlanReject && (
                        <div style={{ marginTop: 10 }}>
                          <textarea
                            ...
                            onChange={(e) =>
                              setPlanRejectReason(e.target.value)
                            }
                            placeholder="Nhập lý do từ chối kế hoạch..."
                            rows={2}
                            style={{
                              width: "100%",
                              borderRadius: 8,
                              ...
                              try {
                                await apiFetch(`/api/tasks/${taskId}/reject`, {
                                  ...
                                  body: JSON.stringify({
                                    ...
                                    overrideRejectTo: "P1.2A",
                                  }),
                                });
                                setSuccessMsg(
                                  "✅ Đã từ chối và đẩy lại task về PM (P1.2A)",
                                );
                                setTimeout(
                  ...
                  {planDecision === "approved" && (
                    ...
                    >
                      ✅ Kế hoạch (PM) đã được phê duyệt
                    </div>
                  )}
                </div>

                {/* Section 2: KTKH Estimate (from P1.2) */}
                <div
                  className="card"
                  style={{
                    padding: "1.5rem",
                ...
                >
                  ...
                  {previousStepData.estimate ? (
                    (() => {
                      ...
                      const renderEstTable = (
                        ...
                        let rows: DtRow[] = [];
                        try {
                          rows = dataStr ? JSON.parse(dataStr) : [];
                        } catch {
                          rows = [];
                        }
                        ...
                        return (
                          ...
                          >
                            ...
                            <div
                              style={{
                                display: "grid",
                                gridTemplateColumns: `30px ${columns.map((c) => c.width || "1fr").join(" ")}`,
                                gap: 4,
                                padding: "5px 8px",
                      ...
                      const est = previousStepData.estimate as Record<
                        string,
                        string
                      >;
                      return (
                        <>
                          ...
                          >
                            {est.totalEstimate ? (
                              ...
                              >
                                ...
                                >
                                  {Number(est.totalEstimate).toLocaleString()}{" "}
                                  VND
                                </strong>
                              </div>
                            ) : est.estimatedBudget ? (
                              ...
                              >
                                ...
                                >
                                  {Number(est.estimatedBudget).toLocaleString()}{" "}
                                  {est.currency || "VND"}
                                </strong>
                              </div>
                            ) : (
                            ...
                            {est.materialCost && (
                              <div>
                                ...
                                <strong>
                                  {Number(est.materialCost).toLocaleString()}
                                </strong>
                              </div>
                            )}
                            {est.laborCost && (
                              <div>
                                ...
                                <strong>
                                  {Number(est.laborCost).toLocaleString()}
                                </strong>
                              </div>
                            )}
                          </div>
                          ...
                          {renderEstTable(
                            ...
                            [
                              ...
                              { key: "tyLe", label: "Tỷ lệ %", width: "0.5fr" },
                            ],
                          )}
                          {renderEstTable(
                            "DT03 — Vật tư tổng hợp",
                            est.dt03Items,
                            [
                              {
                                key: "nhomVT",
                              ...
                              { key: "kl", label: "KL/SL", width: "0.6fr" },
                              {
                                key: "donGia",
                                label: "Đơn giá",
                                width: "0.7fr",
                          ...
                          {renderEstTable(
                            ...
                            [
                              ...
                              { key: "kl", label: "KL/SL", width: "0.5fr" },
                              {
                                key: "donGia",
                                label: "Đơn giá",
                                width: "0.7fr",
                    ...
                    >
                      Chưa có Bảng dự toán từ KTKH. Phải chờ KTKH (P1.2) hoàn
                      tất.
                    </div>
                  )}
                    ...
                    previousStepData.estimate && (
                      <div style={{ marginTop: 16 }}>
                        <div style={{ display: "flex", gap: 10 }}>
                            ...
                            onClick={async () => {
                              ...
                              try {
                                ...
                                if (res.ok && res.task) {
                                  ...
                                  setSuccessMsg(
                                    "✅ Đã duyệt Dự toán (KTKH). Vui lòng duyệt Kế hoạch (PM) nếu chưa duyệt.",
                                  );
                                } else {
                                  setError(res.error || "Lỗi lưu thông tin");
                                }
                              } catch {
                                setError("Lỗi khi duyệt dự toán");
                              }
                              setSubmitting(false);
                            }}
                            disabled={submitting}
                          ...
                          <button
                            onClick={() =>
                              setShowEstimateReject(!showEstimateReject)
                            }
                            disabled={submitting}
                            style={{
                              padding: "8px 20px",
                        ...
                        {showEstimateReject && (
                          <div style={{ marginTop: 10 }}>
                            <textarea
                              ...
                              onChange={(e) =>
                                setEstimateRejectReason(e.target.value)
                              }
                              placeholder="Nhập lý do từ chối dự toán..."
                              rows={2}
                              style={{
                                width: "100%",
                                borderRadius: 8,
                                  ...
                                  await apiFetch(
                                    `/api/tasks/${taskId}/reject`,
                                    {
                                      ...
                                      body: JSON.stringify({
                                        ...
                                        overrideRejectTo: "P1.2",
                                      }),
                                    },
                                  );
                                  setSuccessMsg(
                                    "✅ Đã từ chối và đẩy lại báo cáo dự toán về KTKH (P1.2)",
                                  );
                                  setTimeout(
                                    () => router.push("/dashboard/tasks"),
                                    2000,
                  ...
                  {estimateDecision === "approved" && (
                    ...
                    >
                      ✅ Dự toán (KTKH) đã được phê duyệt
                    </div>
                  )}
                </div>
                    ...
                    >
                      <div style={{ marginBottom: 16 }}>
                        ...
                        <h3 style={{ margin: "8px 0", color: "#16a34a" }}>
                          Sẵn sàng phê duyệt toàn bộ Dự án
                        </h3>
                        <p
                          style={{
                            fontSize: "0.9rem",
                        ...
                        >
                          Cả <strong>Kế hoạch PM</strong> và{" "}
                          <strong>Dự toán KTKH</strong> đều đã được duyệt.
                        </p>
                      </div>
                      <button
                      ...
                      >
                        {submitting
                          ? "⏳ Đang xử lý..."
                          : "Đóng dấu XÁC NHẬN - Hoàn thành P1.3"}
                      </button>
                    </div>
                  )}
              </>
              ...
              <>
                  ...
                  (() => {
                    ...
                    const calcTotal = (
                      ...
                      const total = FAB_STAGES.reduce((sum, s) => {
                        if (overrideKey && s.key === overrideKey)
                          return sum + (overrideVal ?? 0);
                        const d = !!formData[`fab_${s.key}_done`];
                        return (
                          sum +
                          (d
                            ? 100
                            : Number(formData[`fab_${s.key}_progress`] || 0))
                        );
                      }, 0);
                        ...
                        >
                          {FAB_STAGES.map((stage, idx) => {
                            ...
                            const progress = isDone
                              ? 100
                              : Number(
                                  formData[`fab_${stage.key}_progress`] || 0,
                                );
                            const progressColor =
                              progress >= 100
                                ? "#16a34a"
                            ...
                            return (
                              ...
                              >
                                  ...
                                  >
                                    <input
                                      ...
                                      onChange={() => {
                                        ...
                                        handleFieldChange(
                                          `fab_${stage.key}_done`,
                                          newDone ? "1" : "",
                                        );
                                        if (newDone) {
                                          handleFieldChange(
                                            `fab_${stage.key}_progress`,
                                            "100",
                                          );
                                          if (
                                            !checklistState[`fab_${stage.key}`]
                                          )
                                ...
                                {isDone && (
                                  ...
                                  >
                                    ✅ 100% — Hoàn thành{" "}
                                    {formData[`fab_${stage.key}_qty`]
                                      ? `(KL: ${formData[`fab_${stage.key}_qty`]})`
                                      : ""}
                                  </div>
                                )}
                ...
                {task.stepCode === "P6.2" &&
                  previousStepData?.budgetTotal != null &&
                  (() => {
                    ...
                    return (
                      ...
                      >
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            Tổng dự toán từ P1.2 (DT03-DT07)
                          </div>
                          <div
                            style={{
                              fontSize: "1.3rem",
                ...
                {!["P5.4", "P1.2", "P2.1A", "P2.1B", "P2.1C", "P2.4"].includes(
                  task.stepCode,
                ) && (
                  <div
                    className="card"
                    style={{
                      padding: "1.5rem",
                      marginTop: task.stepCode === "P5.1" ? "1rem" : undefined,
                    }}
                  >
                    <h3
                      style={{
                    ...
                    >
                      {config.fields.map((field) => {
                        const isAutoCalc =
                          task.stepCode === "P5.1" &&
                          field.key === "fabricationProgress";
                        // P6.2: auto-calc fields
                        const isP62Total =
                          task.stepCode === "P6.2" &&
                          field.key === "totalActualCost";
                        const isP62Variance =
                          task.stepCode === "P6.2" &&
                          field.key === "costVariance";
                        const isP62CostField =
                          task.stepCode === "P6.2" &&
                          [
                            "actualMaterialCost",
                            "actualLaborCost",
                        ...
                        const isP63Profit =
                          task.stepCode === "P6.3" &&
                          field.key === "grossProfit";
                        const isP63Margin =
                          task.stepCode === "P6.3" &&
                          field.key === "profitMargin";
                        const isReadonlyCalc =
                          isAutoCalc ||
                          isP62Total ||
                          isP62Variance ||
                          isP63Profit ||
                        ...
                        const calcP62 = (
                          ...
                          const budget = Number(
                            previousStepData?.budgetTotal || 0,
                          );
                          if (budget > 0) {
                            const variance = (
                              ((total - budget) / budget) *
                        ...
                        const getCalcColor = () => {
                          ...
                          if (isAutoCalc)
                            return Number(formData.fabricationProgress || 0) >=
                              100
                              ? "#16a34a"
                              : "#f59e0b";
                        ...
                        return field.type === "section" ? (
                          <div
                            ...
                            style={{
                              ...
                              paddingBottom: 6,
                              borderBottom:
                                "2px solid var(--accent-light, #c7d2fe)",
                            }}
                          >
                            <span
                              style={{
                          ...
                          >
                            ...
                            {isReadonlyCalc ? (
                              ...
                              renderField(
                                field,
                                formData[field.key] ?? "",
                                (v) => handleFieldChange(field.key, v),
                                isActive,
                              )
                            )}
                          </div>
                ...
                {task.stepCode === "P5.2" &&
                  previousStepData?.jobCardData &&
                  (() => {
                    const jc = previousStepData.jobCardData as Record<
                      string,
                      string
                    >;
                        ...
                        <div style={{ textAlign: "center" }}>
                          <div
                            style={{
                              ...
                              fontWeight: 800,
                              color:
                                totalProgress >= 100 ? "#16a34a" : "#f59e0b",
                            }}
                          >
                            {totalProgress}%
                          </div>
                ...
                {task.stepCode === "P1.2" &&
                  (() => {
                    ...
                    const renderEstTable = (
                      ...
                      let rows: EstRow[] = [];
                      try {
                        const p = formData[dataKey]
                          ? JSON.parse(formData[dataKey] as string)
                          : null;
                        rows =
                          Array.isArray(p) && p.length > 0 ? p : defaultRows;
                      } catch {
                        rows = defaultRows;
                      }
                      const save = (next: EstRow[]) =>
                        handleFieldChange(dataKey, JSON.stringify(next));
                      const addRow = () =>
                        save([
                          ...rows,
                        ...
                        >
                          ...
                          >
                            <div
                              style={{
                                display: "grid",
                                gridTemplateColumns: `30px ${columns.map((c) => c.width || "1fr").join(" ")} ${isActive ? "28px" : ""}`,
                                gap: 4,
                                padding: "6px 8px",
                                background: "var(--bg-secondary)",
                                borderBottom: "1px solid var(--border)",
                                  ...
                                  if (c.key.toLowerCase() === "dvt") {
                                    return (
                                      <select
                                        ...
                                        disabled={!isActive}
                                        onChange={(e) =>
                                          update(ri, c.key, e.target.value)
                                        }
                                        style={{
                                          fontSize: "0.75rem",
                                          padding: "3px 2px",
                    ...
                    return (
                      <>
                        {/* ── TM: DT02 - Tổng hợp dự toán thi công ── */}
                        {renderEstTable(
                          "📊 DT02 — Tổng hợp dự toán thi công",
                          "QT30-DT02",
                          "dt02Items",
                          [
                            { key: "maCP", label: "Mã CP", width: "0.6fr" },
                        ...
                        {renderEstTable(
                          ...
                          [
                            ...
                            {
                              nhomVT: "VTP",
                              danhMuc: "Vật tư phụ kiện, bu lông…",
                              dvt: "",
                              kl: "",
                              donGia: "",
                              thanhTien: "",
                        ...
                        {renderEstTable(
                          ...
                          [
                            ...
                            {
                              maCP: "NDT",
                              noiDung: "NDT, quy trình và thí nghiệm",
                              dvt: "",
                              kl: "",
                              donGia: "",
                              thanhTien: "",
                        ...
                        {renderEstTable(
                          "👷 DT06 — Dự toán chi phí nhân công",
                          "QT30-DT06",
                          "dt06Items",
                          [
                            { key: "maCP", label: "Mã CP", width: "0.6fr" },
                          ...
                          [
                            ...
                            {
                              maCP: "VS",
                              noiDung: "Vệ sinh vật liệu hợp kim",
                              dvt: "Kg",
                              kl: "",
                              donGia: "",
                              thanhTien: "",
                            ...
                            {
                              maCP: "LTB",
                              noiDung: "Lắp thiết bị phụ kiện trước đóng kiện",
                              dvt: "Bộ",
                              kl: "",
                              donGia: "",
                              thanhTien: "",
                ...
                {task.stepCode === "P1.2A" &&
                  (() => {
                    ...
                    const addRow = () =>
                      save([
                        ...rows,
                        { ...emptyRow(), stt: String(rows.length + 1) },
                      ]);
                    const removeRow = (i: number) =>
                      save(rows.filter((_, idx) => idx !== i));
                    const update = (i: number, key: string, val: string) => {
                    ...
                    const subCols = [
                      { key: "cutting", label: "Cắt" },
                      { key: "machining", label: "GCCK" },
                      { key: "fitup", label: "Gá" },
                      { key: "welding", label: "Hàn" },
                      { key: "tryAssembly", label: "Tổ hợp" },
                      { key: "dismantle", label: "Tháo dỡ" },
                      { key: "blasting", label: "Làm sạch" },
                      { key: "painting", label: "Sơn" },
                      { key: "insulation", label: "Bảo ôn" },
                      { key: "commissioning", label: "Chạy thử" },
                      { key: "packing", label: "Đóng kiện" },
                      { key: "delivery", label: "Giao hàng" },
                    ];
                    const activeSubCount = (row: WbsRow) =>
                      subCols.filter((c) => row[c.key] === "X").length;
                    ...
                    const exportExcel = () => {
                      ...
                      XLSX.writeFile(
                        wb,
                        `WBS_${task.project?.projectCode || "export"}.xlsx`,
                      );
                    };
                      ...
                      input.onchange = (e) => {
                        ...
                        reader.onload = (evt) => {
                          ...
                          const keyMap: Record<string, string> = {
                            ...
                            no: "stt",
                            "no.": "stt",
                            "tên hạng mục": "hangMuc",
                            "tên công trình, hạng mục": "hangMuc",
                            "hạng mục": "hangMuc",
                            item: "hangMuc",
                            description: "hangMuc",
                            "project name": "hangMuc",
                            đvt: "dvt",
                            unit: "dvt",
                          ...
                          const colMapping = headerRow.map(
                            (h) => keyMap[h] || "",
                          );
                          ...
                          for (let i = 1; i < jsonData.length; i++) {
                            ...
                            if (!newRow.stt)
                              newRow.stt = String(imported.length + 1);
                      ...
                      >
                        ...
                        >
                          <div>
                            ...
                            >
                              📋 Bảng kế hoạch tổng thể triển khai (WBS)
                            </h3>
                            <p
                              style={{
                                margin: "4px 0 0",
                            ...
                            >
                              Biểu mẫu BCTH-IBSHI-QLDA-095 — Bấm ▶ để mở phạm vi
                              thầu phụ
                            </p>
                          </div>
                        ...
                        >
                          ...
                          <div
                            style={{
                              display: "grid",
                              gridTemplateColumns:
                                "28px 36px 1.8fr 50px 70px 55px 80px 90px 90px 70px" +
                                (isActive ? " 28px" : ""),
                              gap: 0,
                              background: "var(--bg-secondary)",
                              ...
                              <div key={ri}>
                                ...
                                >
                                  ...
                                  <input
                                    ...
                                    disabled={!isActive}
                                    onChange={(e) =>
                                      update(ri, "hangMuc", e.target.value)
                                    }
                                  ...
                                  <input
                                    ...
                                    disabled={!isActive}
                                    onChange={(e) =>
                                      update(ri, "khoiLuong", e.target.value)
                                    }
                                    placeholder="KL"
                                    style={{
                                      fontSize: "0.7rem",
                                  ...
                                  <select
                                    className="input"
                                    value={row.phamVi || "IBS"}
                                    disabled={!isActive}
                                    onChange={(e) =>
                                      update(ri, "phamVi", e.target.value)
                                    }
                                    style={{
                                      fontSize: "0.65rem",
                                      padding: "1px",
                                  ...
                                  <input
                                    ...
                                    disabled={!isActive}
                                    onChange={(e) =>
                                      update(ri, "thauPhu", e.target.value)
                                    }
                                  ...
                                  <input
                                    ...
                                    disabled={!isActive}
                                    onChange={(e) =>
                                      update(ri, "batDau", e.target.value)
                                    }
                                    style={{
                                      fontSize: "0.65rem",
                                      padding: "1px 2px",
                                  ...
                                  <input
                                    ...
                                    disabled={!isActive}
                                    onChange={(e) =>
                                      update(ri, "ketThuc", e.target.value)
                                    }
                                    style={{
                                      fontSize: "0.65rem",
                                      padding: "1px 2px",
                                  ...
                                  <input
                                    ...
                                    disabled={!isActive}
                                    onChange={(e) =>
                                      update(ri, "trangThai", e.target.value)
                                    }
                                    placeholder="TT"
                                    style={{
                                      fontSize: "0.68rem",
                                ...
                                {isExpanded && (
                                  ...
                                  >
                                    ...
                                    >
                                      Phạm vi công việc chi tiết thầu phụ:
                                    </div>
                                    <div
                                      style={{
                                        display: "grid",
                                    ...
                                    >
                                      <div>
                                        ...
                                        <input
                                          ...
                                          disabled={!isActive}
                                          onChange={(e) =>
                                            update(ri, "khuVuc", e.target.value)
                                          }
                                          placeholder="VD: Xưởng 1, ngoài bãi..."
                                          style={{
                                            fontSize: "0.75rem",
                                            padding: "4px 8px",
                                            width: "100%",
                                      ...
                                      <div>
                                        ...
                                        <input
                                          ...
                                          disabled={!isActive}
                                          onChange={(e) =>
                                            update(ri, "ghiChu", e.target.value)
                                          }
                                          placeholder="Ghi chú..."
                                          style={{
                                            fontSize: "0.75rem",
                ...
                {task.stepCode === "P3.1" &&
                  (() => {
                    ...
                    let llItems: LlItem[] = [];
                    try {
                      const p = formData[llKey]
                        ? JSON.parse(formData[llKey] as string)
                        : null;
                      llItems =
                        Array.isArray(p) && p.length > 0 ? p : defaultLl;
                    } catch {
                      llItems = defaultLl;
                    }
                    const saveLl = (next: LlItem[]) =>
                      handleFieldChange(llKey, JSON.stringify(next));
                    const addLl = () =>
                      saveLl([
                        ...llItems,
                        {
                    ...
                    const removeLl = (i: number) =>
                      saveLl(llItems.filter((_, idx) => idx !== i));
                    const updateLl = (i: number, key: string, val: string) => {
                      const n = [...llItems];
                      n[i] = { ...n[i], [key]: val } as LlItem;
                      saveLl(n);
                    };
                    ...
                    return (
                      <>
                        ...
                        >
                          ...
                          >
                            Dữ liệu chỉ đọc — PM xem xét để xác định vật tư
                            long-lead
                          </p>
                          {wbsRows.length === 0 ? (
                            <p
                            ...
                            >
                              Chưa có dữ liệu WBS từ bước P1.2A
                            </p>
                          ) : (
                            <div
                              style={{
                        ...
                        >
                          ...
                          >
                            <div>
                              ...
                              >
                                🔴 Danh sách vật tư Long-Lead cần ưu tiên
                              </h3>
                              <p
                                style={{
                                  margin: "4px 0 0",
                              ...
                              >
                                Xác định vật tư cần đặt gấp, ngày cần có, mức ưu
                                tiên
                              </p>
                            </div>
                            {isActive && (
                            ...
                            {llItems.map((item, i) => (
                              ...
                              >
                                <input
                                  ...
                                  disabled={!isActive}
                                  onChange={(e) =>
                                    updateLl(i, "material", e.target.value)
                                  }
                                  placeholder="VD: Thép tấm SS400, Q345B..."
                                  style={{
                                    fontSize: "0.78rem",
                                    padding: "4px 8px",
                                    width: "100%",
                                ...
                                <input
                                  ...
                                  disabled={!isActive}
                                  onChange={(e) =>
                                    updateLl(i, "dateNeeded", e.target.value)
                                  }
                                  style={{
                                    fontSize: "0.72rem",
                                    padding: "3px 4px",
                                ...
                                <select
                                  ...
                                  disabled={!isActive}
                                  onChange={(e) =>
                                    updateLl(i, "priority", e.target.value)
                                  }
                                  style={{
                                    fontSize: "0.72rem",
                                    padding: "3px 4px",
                                    width: "100%",
                                    color:
                                      priorityColors[item.priority] || "#333",
                                    fontWeight: 600,
                                  }}
                                >
                                ...
                                <input
                                  ...
                                  disabled={!isActive}
                                  onChange={(e) =>
                                    updateLl(i, "note", e.target.value)
                                  }
                                  placeholder="Ghi chú..."
                                  style={{
                                    fontSize: "0.75rem",
                          ...
                          >
                            {llItems.length} vật tư long-lead
                          </div>
                        </div>
                      </>
                    );
                ...
                {["P1.2", "P2.1A", "P2.4"].includes(task.stepCode) &&
                  (() => {
                    const getInheritedEstRows = (
                      key: string,
                      baseDefault: any[],
                    ) => {
                      if (
                    ...
                    const renderEstTable = (
                      ...
                      const exportEstExcel = () => {
                        ...
                        XLSX.writeFile(
                          wb,
                          `${code}_${task?.project?.projectCode || "P2"}.xlsx`,
                        );
                      };
                      ...
                      const importEstExcel = () => {
                        ...
                        input.onchange = (e) => {
                          ...
                          reader.onload = (evt) => {
                            ...
                            for (let i = 1; i < jsonData.length; i++) {
                              const rowData = jsonData[i];
                              if (!rowData || rowData.every((c) => !c))
                                continue;
                              const newRow: EstRow = Object.fromEntries(
                                columns.map((c) => [c.key, ""]),
                              );
                              colMapping.forEach((key, ci) => {
                                if (key && rowData[ci] != null)
                            ...
                            if (imported.length > 0) {
                              save([...rows, ...imported]);
                              setSuccessMsg(
                                `✅ Đã import ${imported.length} mục cho ${code}`,
                              );
                              setTimeout(() => setSuccessMsg(""), 3000);
                            }
                          };
                        ...
                        <>
                          {siblingFiles?.["file_contract"] && (
                            ...
                            >
                              ...
                              >
                                <a
                                  href={siblingFiles["file_contract"] as string}
                                  target="_blank"
                                ...
                                >
                                  ⬇️{" "}
                                  {decodeURIComponent(
                                    (siblingFiles["file_contract"] as string)
                                      .split("/")
                                      .pop() || "Hợp đồng",
                                  ).replace(/^file_contract_?/, "")}
                                </a>
                              </div>
                            </div>
                          )}
                          {renderEstTable(
                            "🏢 DT07 — Chi phí chung, chi phí tài chính",
                            "QT30-DT07",
                            "dt07Items",
                            colsCPChung,
                            [
                              {
                                maCP: "CPC",
                                danhMuc: "I. Chi phí chung phục vụ sản xuất",
                                dvt: "",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                              ...
                              {
                                maCP: "CPC-02",
                                danhMuc: "Thuê công nhân thời vụ",
                                dvt: "Người",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                              ...
                              {
                                maCP: "CPC-04",
                                danhMuc: "Sửa chữa máy móc thiết bị",
                                dvt: "",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                              ...
                              {
                                maCP: "CPC-09",
                                danhMuc: "Chi phí an toàn lao động",
                                dvt: "",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                              ...
                              {
                                maCP: "CTC-01",
                                danhMuc: "Phí bảo lãnh thực hiện HĐ",
                                dvt: "",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                              ...
                              {
                                maCP: "CQL-01",
                                danhMuc: "Lương nhân viên gián tiếp",
                                dvt: "Người",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                    ...
                    if (task.stepCode === "P2.1B")
                      return (
                        <>
                          {renderEstTable(
                            "📦 DT03 — Dự toán chi phí VT tổng hợp",
                            "QT30-DT03",
                            "dt03Items",
                            colsVTTH,
                            [
                              ...
                              {
                                nhomVT: "VTBP-01",
                                danhMuc: "Thép đen các loại (biện pháp)",
                                dvt: "Kg",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                              },
                              {
                                nhomVT: "VTBP-TH",
                                danhMuc:
                                  "Thu hồi lại vật tư biện pháp (75% giá trị)",
                                dvt: "Kg",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                          ...
                          {renderEstTable(
                            ...
                            [
                              ...
                              {
                                maVT: "A2",
                                tenVT: "Vật tư còn lại (chưa có chi tiết)",
                                macVL: "",
                                quyCach: "",
                                dvt: "",
                                kl: "",
                              ...
                              {
                                maVT: "B",
                                tenVT: "VT04 — Vật tư phụ kiện, bu lông…",
                                macVL: "",
                                quyCach: "",
                                dvt: "",
                                kl: "",
                              ...
                              {
                                maVT: "D",
                                tenVT: "VTBP — Vật tư làm biện pháp",
                                macVL: "",
                                quyCach: "",
                                dvt: "",
                                kl: "",
                    ...
                    // ── P2.1C: SX (DT06) ──
                    if (task.stepCode === "P2.1C")
                      return (
                        <>
                          {renderEstTable(
                            "👷 DT06 — Dự toán chi phí nhân công trực tiếp",
                            "QT30-DT06",
                            "dt06Items",
                            colsNC,
                            [
                              ...
                              {
                                maCP: "B-03",
                                noiDung: "Vát mép, chuẩn bị mối hàn",
                                dvt: "Kg",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                              ...
                              {
                                maCP: "F",
                                noiDung: "LD — LẮP DỰNG + NGHIỆM THU",
                                dvt: "",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                              ...
                              {
                                maCP: "G-01",
                                noiDung: "Vệ sinh VL hợp kim bằng dung dịch",
                                dvt: "Kg",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                              ...
                              {
                                maCP: "H-01",
                                noiDung: "Làm sạch bề mặt (bi, cát)",
                                dvt: "m²",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                              ...
                              },
                              {
                                maCP: "J-01",
                                noiDung:
                                  "Lắp thiết bị phụ kiện trước đóng kiện",
                                dvt: "Bộ",
                                kl: "",
                                donGia: "",
                                thanhTien: "",
                    ...
                    return (
                      <>
                        ...
                        >
                          ...
                          >
                            {(task.stepCode as string) === "P1.2"
                              ? "Lập dự toán thi công (P1.2)."
                              : (task.stepCode as string) === "P2.1A"
                                ? "Bảng tính các định mức và dự toán từ phòng TCKT (P2.1A)."
                        ...
                        {task.stepCode === "P2.4" &&
                          renderEstTable(
                            "📊 DT02 — Tổng hợp chi phí dự toán thi công",
                            "QT30-DT02",
                            "dt02Items",
                            [
                              { key: "maCP", label: "Mã CP", width: "0.6fr" },
                ...
                {task.stepCode === "P2.4" && previousStepData && (
                  <>
                          ...
                          {previousStepData.estimate.estimatedBudget && (
                            <div>
                              ...
                              <strong style={{ color: "#16a34a" }}>
                                ...
                                {previousStepData.estimate.currency || "VND"}
                              </strong>
                            </div>
                          )}
                    ...
                    {[
                      {
                        key: "bomMain",
                        label: "📦 VT chính — Thiết kế (P2.1)",
                        color: "#3b82f6",
                      },
                      {
                        key: "bomWeldPaint",
                        label: "🔥 VT hàn & sơn — PM (P2.2)",
                        color: "#ef4444",
                      },
                      {
                        key: "bomSupply",
                        label: "📋 VT phụ — Kho (P2.3)",
                        color: "#10b981",
                      },
                    ].map((section) => {
                      const data = previousStepData[
                        section.key as keyof typeof previousStepData
                            ...
                            <>
                              <div
                                style={{
                                  display: "grid",
                                  gridTemplateColumns:
                                    "40px 1.5fr 1fr 1.2fr 0.7fr 0.6fr",
                                  gap: 6,
                                  padding: "6px 4px",
                                  borderBottom: "2px solid var(--border)",
                                  marginBottom: 4,
                  ...
                  <>
                    ...
                    {previousStepData.plan && (
                      ...
                      >
                        ...
                        >
                          📋 Kế hoạch SX & Dự toán điều chỉnh (từ KTKH — P2.4)
                        </h3>
                        <div
                          style={{
                            display: "grid",
                        ...
                        {previousStepData.plan.productionPlan && (
                          ...
                          >
                            ...
                            >
                              {String(previousStepData.plan.productionPlan)}
                            </div>
                          </div>
                        )}
                        {previousStepData.plan.budgetImpact && (
                          ...
                          >
                            ...
                            <div
                              style={{ marginTop: 4, whiteSpace: "pre-wrap" }}
                            >
                              {String(previousStepData.plan.budgetImpact)}
                            </div>
                          </div>
                        )}
                        {previousStepData.plan.workshopTimeline && (
                          ...
                          >
                            ...
                            <div
                              style={{ marginTop: 4, whiteSpace: "pre-wrap" }}
                            >
                              {String(previousStepData.plan.workshopTimeline)}
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    {previousStepData.plan &&
                      (() => {
                        ...
                        const renderReadonlyTable = (
                          ...
                          let rows: DtRow[] = [];
                          try {
                            rows = dataStr ? JSON.parse(dataStr as string) : [];
                          } catch {
                            rows = [];
                          }
                        ...
                        return (
                          <div style={{ marginTop: "1rem" }}>
                            ...
                            >
                              ...
                              >
                                📊 Dự toán chi tiết từ các phòng ban
                              </h3>
                            </div>
                            {renderReadonlyTable(
                              "DT02 — Tổng hợp chi phí",
                              ...
                              de.dt02Items ||
                                (
                                  previousStepData.estimate as Record<
                                    string,
                                    string
                                  >
                                )?.dt02Items,
                        ...
                        >
                          ...
                          <div
                            style={{
                              display: "grid",
                              gridTemplateColumns:
                                "35px 1.5fr 1fr 1.2fr 0.6fr 0.5fr",
                              gap: 4,
                              padding: "4px 2px",
                              borderBottom: "1px solid var(--border)",
                              marginBottom: 2,
                  ...
                  <>
                    ...
                    >
                      <div style={{ fontSize: "0.85rem" }}>
                        📊 Tổng PR:{" "}
                        <strong>
                          {(previousStepData.prItems as unknown[])?.length || 0}
                        </strong>{" "}
                        mục
                      </div>
                      <div style={{ fontSize: "0.85rem", color: "#16a34a" }}>
                        ✅ Xuất kho:{" "}
                        <strong>
                          {(previousStepData.fromStock as unknown[])?.length ||
                            0}
                        </strong>
                      </div>
                      <div style={{ fontSize: "0.85rem", color: "#dc2626" }}>
                        🛒 Cần mua:{" "}
                        <strong>
                          {(previousStepData.toPurchase as unknown[])?.length ||
                            0}
                        </strong>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      >
                        ✅ Xuất từ kho (Tồn đủ + Quy chuẩn OK)
                      </h3>
                      {(
                        previousStepData.fromStock as {
                          name: string;
                          code: string;
                          spec: string;
                          quantity: string;
                        ...
                        <>
                          <div
                            style={{
                              display: "grid",
                              gridTemplateColumns:
                                "30px 1.2fr 0.8fr 0.8fr 0.6fr 0.6fr 0.6fr 0.5fr",
                              gap: 4,
                              padding: "4px 2px",
                              borderBottom: "2px solid var(--border)",
                              marginBottom: 2,
                    ...
                    >
                      ...
                      >
                        🛒 Cần mua (Không đủ tồn / Quy chuẩn không đạt)
                      </h3>
                      {(
                        previousStepData.toPurchase as {
                          name: string;
                          code: string;
                          spec: string;
                          quantity: string;
                        ...
                        <>
                          <div
                            style={{
                              display: "grid",
                              gridTemplateColumns:
                                "30px 1.2fr 0.8fr 0.8fr 0.5fr 0.5fr 0.5fr 0.5fr 0.5fr",
                              gap: 4,
                              padding: "4px 2px",
                              borderBottom: "2px solid var(--border)",
                              marginBottom: 2,
                ...
                {task.stepCode === "P3.5" &&
                  (() => {
                    ...
                    const prItems: PrItem[] = Array.isArray(prItemsRaw)
                      ? prItemsRaw.filter((p: PrItem) => p.name?.trim())
                      : [];
                    ...
                    return (
                      <>
                        ...
                        >
                          ...
                          >
                            <div>
                              ...
                              >
                                {prItems.length > 0
                                  ? `${prItems.length} vật tư PR từ BOM (P2.1/P2.2/P2.3) — TM chỉ điền giá cho từng NCC`
                                  : "Chưa có vật tư PR — TM tự nhập vật tư"}
                              </p>
                            </div>
                            {isActive && (
                              <button
                          ...
                          {suppliers.map((supplier, sIdx) => (
                            <div
                              ...
                              style={{
                                ...
                                padding: "1rem",
                                marginBottom: 16,
                                borderLeft: `4px solid hsl(${sIdx * 60 + 200}, 60%, 50%)`,
                              }}
                            >
                                ...
                                >
                                  ...
                                  <tbody>
                                    {supplier.quotes.map((q, qIdx) => {
                                      ...
                                      const prMatch = prItems.find(
                                        (p) =>
                                          p.name.toLowerCase() ===
                                          q.material.toLowerCase(),
                                      );
                                      return (
                                        <tr
                                          key={qIdx}
                              ...
                              >
                                ...
                                >
                                  {
                                    supplier.quotes.filter(
                                      (q) => q.price && Number(q.price) > 0,
                                    ).length
                                  }
                                  /{supplier.quotes.length} vật tư đã có giá
                                </span>
                                <span
                                  style={{
                                    fontSize: "0.85rem",
                        ...
                        {(() => {
                          const namedSuppliers = suppliers.filter((s) =>
                            s.name.trim(),
                          );
                          const allMaterials = [
                            ...new Set(
                              namedSuppliers.flatMap((s) =>
                                s.quotes
                                  .filter((q) => q.material.trim())
                                  .map((q) => q.material.trim().toLowerCase()),
                              ),
                            ),
                          ];
                          if (
                            namedSuppliers.length < 2 ||
                                  ...
                                  <tbody>
                                    ...
                                    >
                                      ...
                                      {namedSuppliers.map((s, sIdx) => {
                                        const total = s.quotes.reduce(
                                          (sum, q) =>
                                            sum + (Number(q.price) || 0),
                                          0,
                                        );
                                        const allTotals = namedSuppliers
                                          .map((ns) =>
                                            ns.quotes.reduce(
                                              (sm, q) =>
                                                sm + (Number(q.price) || 0),
                                      ...
                                      >
                                        {(() => {
                                          const totals = namedSuppliers.map(
                                            (s) =>
                                              s.quotes.reduce(
                                                (sm, q) =>
                                                  sm + (Number(q.price) || 0),
                ...
                {task.stepCode === "P3.6" &&
                  ...
                  (() => {
                    const sd = previousStepData.supplierData as {
                      suppliers?: {
                        name: string;
                        quotes: { material: string; price: string }[];
                      }[];
                      [key: string]: unknown;
                    };
                    ...
                    return (
                      <>
                        ...
                        >
                          ...
                          >
                            🏭 Danh sách NCC đề xuất từ Thương mại (
                            {namedS.length} NCC)
                          </h3>
                          {namedS.map((s, sIdx) => (
                            <div
                              ...
                              style={{
                                ...
                                padding: "0.75rem",
                                marginBottom: 8,
                                borderLeft: `3px solid hsl(${sIdx * 60 + 200}, 60%, 50%)`,
                              }}
                            >
                              <div
                                style={{
                              ...
                              <div
                                style={{
                                  display: "grid",
                                  gridTemplateColumns: "30px 1.5fr 1fr",
                                  gap: 4,
                                  padding: "2px",
                                  borderBottom: "1px solid var(--border)",
                                  marginBottom: 4,
                              ...
                              {(s.quotes || [])
                                ...
                                .map((q, qIdx) => (
                                  ...
                                  >
                                    ...
                                    >
                                      {Number(q.price).toLocaleString("vi-VN")}{" "}
                                      ₫
                                    </span>
                                  </div>
                                ))}
                  ...
                  <>
                    ...
                    {previousStepData?.supplierData &&
                      (() => {
                        ...
                        const nccList = (sd?.suppliers || []).filter((s) =>
                          s.name?.trim(),
                        );
                        ...
                        type BestItem = {
                          ...
                          bestPrice: number;
                          bestNCC: string;
                          prices: { ncc: string; price: number }[];
                        };
                        const bestItems: BestItem[] = allMats.map((mat) => {
                          const prices = nccList
                            .map((s) => {
                              const q = (s.quotes || []).find(
                                (q) =>
                                  q.material?.trim().toLowerCase() ===
                        ...
                        const grandTotal = bestItems.reduce(
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              Giá tốt nhất cho từng vật tư đã so sánh từ{" "}
                              {nccList.length} NCC
                            </p>
                            <div
                              style={{
                                ...
                                <tbody>
                                  {bestItems.map((item, idx) => (
                                    ...
                                    >
                                      ...
                                      >
                                        {item.bestPrice > 0
                                          ? `${item.bestPrice.toLocaleString("vi-VN")} ₫`
                                          : "—"}
                                      </td>
                      ...
                      {paymentType === "partial" && (
                        ...
                        >
                          ...
                          <div
                            style={{
                              display: "grid",
                              gridTemplateColumns: "0.8fr 0.5fr 1fr 30px",
                              gap: 6,
                              padding: "2px",
                              borderBottom: "1px solid var(--border)",
                              marginBottom: 4,
                          ...
                          >
                            {["Đợt", "% thanh toán", "Ngày dự kiến", ""].map(
                              (h) => (
                                <span
                                  key={h}
                                  style={{
                          ...
                          {paymentMilestones.filter(
                            (p) => Number(p.percent) > 0,
                          ).length > 0 && (
                            <div
                              style={{
                                marginTop: 6,
                    ...
                    >
                      ...
                      <select
                        ...
                        disabled={!isActive}
                        onChange={(e) =>
                          setDeliveryType(e.target.value as "full" | "batch")
                        }
                        style={{
                          fontSize: "0.9rem",
                          fontWeight: 600,
                      ...
                      >
                        <option value="full">
                          📦 Giao hàng toàn bộ (1 lần)
                        </option>
                        <option value="batch">
                          📋 Giao từng lần (nhiều đợt)
                        </option>
                      </select>
                      ...
                      {deliveryType === "batch" && (
                        ...
                        >
                          ...
                          <div
                            style={{
                              display: "grid",
                              gridTemplateColumns: "30px 1.2fr 0.6fr 1fr 30px",
                              gap: 6,
                              padding: "2px",
                              borderBottom: "1px solid var(--border)",
                              marginBottom: 4,
                ...
                {task.stepCode === "P4.1" &&
                  ...
                  (() => {
                    const pd = previousStepData.poData as {
                      paymentType?: string;
                      paymentMilestones?: {
                        label: string;
                        percent: string;
                    ...
                    const milestonesList =
                      pType === "partial" && pd?.paymentMilestones
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          💰 Các đợt thanh toán từ Thương mại
                        </h3>
                        {pd?.poNumber && (
                          <div
                            style={{
                          ...
                          >
                            ...
                            {pd?.totalAmount
                              ? ` — Tổng: ${Number(pd.totalAmount).toLocaleString("vi-VN")} ₫`
                              : ""}
                          </div>
                        )}
                        <div
                        ...
                        >
                          ...
                          <div
                            style={{
                              display: "grid",
                              gridTemplateColumns:
                                "30px 1fr 0.5fr 0.8fr 1fr 0.8fr 30px",
                              gap: 6,
                              padding: "8px 10px",
                              background: "var(--bg-secondary)",
                              borderBottom: "1px solid var(--border)",
                          ...
                          {milestonesList.map((ms, idx) => {
                            const conf = paymentConfirmations[idx] || {
                              confirmed: false,
                              method: "",
                              paidDate: "",
                            };
                            return (
                              ...
                              >
                                ...
                                <input
                                  ...
                                  type="date"
                                  value={
                                    (conf as unknown as { paidDate?: string })
                                      .paidDate || ""
                                  }
                                  disabled={!isActive}
                                  onChange={(e) =>
                        ...
                        >
                          <span>
                            ...
                            <strong style={{ color: "#16a34a" }}>
                              {
                                paymentConfirmations.filter((c) => c.confirmed)
                                  .length
                              }
                          ...
                          <span>
                            ...
                            <strong>
                              {milestonesList
                                .filter(
                                  (_, i) => paymentConfirmations[i]?.confirmed,
                                )
                                .reduce(
                  ...
                  (() => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          📦 Danh sách vật tư theo dõi giao hàng
                        </h3>
                        <div
                          style={{
                            border: "1px solid var(--border)",
                          ...
                          >
                            ...
                            {pd?.totalAmount && (
                              <>
                                ...
                                — 💰 Tổng PO:{" "}
                                <strong>
                                  {Number(pd.totalAmount).toLocaleString(
                                    "vi-VN",
                                  )}{" "}
                                  ₫
                                </strong>
                ...
                {task.stepCode === "P4.4" &&
                  previousStepData &&
                  (() => {
                    const qd = previousStepData.qcData as {
                    ...
                    const materials = allMats.map((mat) => {
                      ...
                      const prItem =
                        prItems.find(
                          (p) => p.name?.trim().toLowerCase() === matLower,
                        ) ||
                        prItems.find(
                          (p) =>
                            p.name?.trim().toLowerCase().includes(matLower) ||
                            matLower.includes(p.name?.trim().toLowerCase()),
                        );
                    ...
                    return (
                      <div
                        ...
                        style={{
                          padding: "1.5rem",
                          marginTop: "1rem",
                          borderLeft: `4px solid ${qcResult === "PASS" ? "#16a34a" : qcResult === "CONDITIONAL" ? "#f59e0b" : "#dc2626"}`,
                        }}
                      >
                        <h3
                          style={{
                            margin: "0 0 6px 0",
                            fontSize: "1rem",
                            color: qcResult === "PASS" ? "#16a34a" : "#f59e0b",
                          }}
                        >
                          📦 Vật tư cần nghiệm thu —{" "}
                          <span
                          ...
                          >
                            ...
                            <div
                              style={{
                                display: "grid",
                                gridTemplateColumns:
                                  "30px 1fr 0.8fr 0.6fr 0.7fr 1fr",
                                gap: 6,
                                padding: "8px 10px",
                                background: "var(--bg-secondary)",
                                borderBottom: "1px solid var(--border)",
                            ...
                            {/* Rows */}
                            {materials.map((m, idx) => {
                              const wi = warehouseItems[idx] || {
                                receivedQty: "",
                                storageLocation: "",
                              };
                              const receivedNum = Number(wi.receivedQty) || 0;
                              const isOverQty =
                                m.prQty > 0 && receivedNum > m.prQty;
                              return (
                                <div key={idx}>
                                  <div
                                    style={{
                                      ...
                                      padding: "8px 10px",
                                      alignItems: "center",
                                      borderBottom: isOverQty
                                        ? "none"
                                        : idx < materials.length - 1
                                          ? "1px solid var(--border)"
                                          : "none",
                                      background: isOverQty
                                        ? "#fef2f2"
                                        : wi.receivedQty && wi.storageLocation
                                          ? "#f0fdf4"
                                          : "transparent",
                                    }}
                                  >
                                  ...
                                  {isOverQty && (
                                    ...
                                    >
                                      ⚠️ SL thực nhận ({receivedNum}) vượt quá
                                      SL PR đã chốt ({m.prQty})
                                    </div>
                                  )}
                                </div>
                              );
                          ...
                          <strong style={{ color: "#16a34a" }}>
                            {
                              warehouseItems.filter(
                                (w) => w.receivedQty && w.storageLocation,
                              ).length
                            }
                            /{materials.length}
                          </strong>{" "}
                ...
                {task.stepCode === "P4.5" &&
                  previousStepData &&
                  (() => {
                    const wd = previousStepData.woData as {
                      woItems?: {
                        costCode: string;
                        content: string;
                        jobCode: string;
                    ...
                    const woItems =
                      wd?.woItems?.filter((w: { content?: string }) =>
                        w.content?.trim(),
                      ) || [];
                    // Parse issueItems from formData
                    const issueItemsRaw = formData.issueItems as
                      | string
                      | undefined;
                    let issueItems: {
                      name: string;
                    ...
                    try {
                      issueItems = issueItemsRaw
                        ? JSON.parse(issueItemsRaw as string)
                        : [];
                    } catch {
                      issueItems = [];
                    }
                      ...
                      <>
                        ...
                        >
                          ...
                          >
                            ...
                            📦 Tồn kho hiện tại ({inv.length} vật tư)
                            <span
                              style={{
                                fontSize: "0.75rem",
                                fontWeight: 400,
                          ...
                          >
                            <div
                              style={{
                                display: "grid",
                                gridTemplateColumns:
                                  "30px 0.7fr 1.2fr 0.8fr 0.6fr 0.5fr",
                                gap: 4,
                                padding: "6px 10px",
                                background: "var(--bg-secondary)",
                                borderBottom: "1px solid var(--border)",
                              ...
                              (
                                ...
                                <div
                                  ...
                                  style={{
                                    padding: "0.75rem",
                                    marginBottom: 8,
                                    borderLeft: `3px solid hsl(${wIdx * 40 + 30}, 70%, 50%)`,
                                  }}
                                >
                                  <div
                                    style={{
                        ...
                        >
                          ...
                          >
                            ...
                            <div>
                              ...
                              <input
                                className="input"
                                placeholder="Nhập WBS Node"
                                value={(formData.wbsNode as string) || ""}
                                disabled={!isActive}
                                onChange={(e) =>
                                  handleFieldChange("wbsNode", e.target.value)
                                }
                                style={{
                                  fontSize: "0.85rem",
                                  padding: "6px 8px",
                          ...
                          <div
                            style={{
                              display: "grid",
                              gridTemplateColumns:
                                "30px 1.2fr 0.8fr 0.8fr 0.6fr 0.5fr 30px",
                              gap: 4,
                              padding: "4px 2px",
                              borderBottom: "2px solid var(--border)",
                              marginBottom: 4,
                ...
                {task.stepCode === "P5.1" &&
                  (() => {
                    ...
                    const qrContent = jcCode
                      ? `https://erp.ibshi.com/drawing/${projectCode}/${jcCode}`
                      : "";
                    return (
                      <div
                        className="card"
                        ...
                        {jcCode ? (
                          ...
                          >
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              >
                                Quét mã QR bằng điện thoại để xem bản vẽ sản
                                xuất mới nhất cho Job Card{" "}
                                <strong
                                  style={{ color: "var(--text-primary)" }}
                                >
                                  {jcCode}
                          ...
                          >
                            ⬆️ Nhập <strong>Mã Job Card</strong> ở trên để tạo
                            mã QR
                          </div>
                        )}
                      </div>
                      ...
                      jobCards = [
                        {
                          code: "",
                          stages: [
                            { hangMuc: "", volume: "", unit: "", team: "" },
                          ],
                        },
                      ];
                    const save = (cards: JobCard[]) =>
                      handleFieldChange("jobCards", JSON.stringify(cards));
                    const updateCard = (
                      ci: number,
                      field: string,
                      val: string,
                    ) => {
                      const next = jobCards.map((c, i) =>
                        i === ci ? { ...c, [field]: val } : c,
                      );
                      save(next);
                    };
                    ...
                    const removeStage = (ci: number, si: number) => {
                      const next = jobCards.map((c, i) =>
                        i === ci
                          ? {
                              ...c,
                              stages: c.stages.filter((_, j) => j !== si),
                            }
                          : c,
                      );
                      save(next);
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            📋 Danh sách Job Card ({jobCards.length})
                          </h3>
                          {isActive && (
                            <button
                              type="button"
                        ...
                        {jobCards.map((card, ci) => (
                          <div
                            key={ci}
                            style={{
                              border: `2px solid ${cardColors[ci % cardColors.length]}25`,
                              borderRadius: 12,
                              marginBottom: 12,
                              overflow: "hidden",
                            }}
                          >
                            ...
                            <div
                              style={{
                                background: `${cardColors[ci % cardColors.length]}10`,
                                padding: "10px 14px",
                                display: "flex",
                                alignItems: "center",
                                gap: 10,
                                borderBottom: `1px solid ${cardColors[ci % cardColors.length]}25`,
                              }}
                            >
                              <span
                                style={{
                                  fontWeight: 700,
                                  fontSize: "0.9rem",
                                  color: cardColors[ci % cardColors.length],
                                }}
                              >
                                JC #{ci + 1}
                              </span>
                              <input
                                className="input"
                                placeholder="Mã Job Card *"
                                value={card.code}
                                disabled={!isActive}
                                onChange={(e) =>
                                  updateCard(ci, "code", e.target.value)
                                }
                                style={{
                                  fontSize: "0.85rem",
                                  padding: "5px 8px",
                            ...
                            <div style={{ padding: "8px 14px" }}>
                                ...
                                {isActive && (
                                  <button
                                    ...
                                    style={{
                                      ...
                                      color: cardColors[ci % cardColors.length],
                                      background: "none",
                                      border: `1px dashed ${cardColors[ci % cardColors.length]}`,
                                      borderRadius: 4,
                                      padding: "2px 8px",
                                      cursor: "pointer",
                                    }}
                              ...
                              <div
                                style={{
                                  display: "grid",
                                  gridTemplateColumns:
                                    "28px 1.2fr 1fr 0.8fr 0.7fr 28px",
                                  gap: 4,
                                  padding: "3px 0",
                                  borderBottom: "1px solid var(--border)",
                                  marginBottom: 3,
                ...
                {task.stepCode === "P5.3" &&
                  (() => {
                    ...
                    let qcItems: QcItem[] = [];
                    try {
                      qcItems = raw ? JSON.parse(raw) : [];
                    } catch {
                      qcItems = [];
                    }
                    if (qcItems.length === 0)
                      qcItems = [{ task: "", result: "" }];
                    const saveQc = (items: QcItem[]) =>
                      handleFieldChange("qcItems", JSON.stringify(items));
                    ...
                    return (
                      <div
                        ...
                        style={{
                          padding: "1.5rem",
                          marginTop: "1rem",
                          borderLeft: `4px solid ${hasFail ? "#dc2626" : "#8b5cf6"}`,
                        }}
                      >
                        <div
                          style={{
                        ...
                        <div
                          style={{
                            display: "grid",
                            gridTemplateColumns: "30px 1.5fr 1fr 30px",
                            gap: 6,
                            padding: "4px 0",
                            borderBottom: "2px solid var(--border)",
                            marginBottom: 4,
                        ...
                        >
                          {["#", "Công việc kiểm tra", "Kết quả", ""].map(
                            (h) => (
                              <span
                                key={h}
                                style={{
                        ...
                        {qcItems.map((item, idx) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <input
                                className="input"
                                placeholder="Mô tả công việc cần kiểm tra"
                                value={item.task}
                                disabled={!isActive}
                                onChange={(e) => {
                                  const next = [...qcItems];
                              ...
                              {isActive && qcItems.length > 1 && (
                                <button
                                  type="button"
                                  onClick={() =>
                                    saveQc(qcItems.filter((_, i) => i !== idx))
                                  }
                                  style={{
                                    background: "none",
                                    border: "none",
                        ...
                        <div
                          style={{
                            marginTop: 10,
                            padding: "8px 12px",
                            background: hasFail ? "#fef2f2" : "#f0fdf4",
                            borderRadius: 8,
                            fontSize: "0.8rem",
                            display: "flex",
                            alignItems: "center",
                          ...
                          {hasFail ? (
                            <>
                              ...
                              >
                                ⚠️ Có{" "}
                                {
                                  qcItems.filter((q) => q.result === "FAIL")
                                    .length
                                }{" "}
                                hạng mục FAIL
                              </span>
                              <span style={{ color: "#dc2626" }}>
                                — Chỉ có thể Từ chối → trả về P5.1
                              </span>
                            </>
                          ) : (
                            <span style={{ color: "#16a34a", fontWeight: 600 }}>
                              ✅{" "}
                              {
                                qcItems.filter((q) => q.result === "PASS")
                                  .length
                              }{" "}
                              PASS,{" "}
                              {
                                qcItems.filter(
                                  (q) => q.result === "CONDITIONAL",
                                ).length
                              }{" "}
                              CONDITIONAL
                            </span>
                ...
                {task.stepCode === "P5.4" &&
                  previousStepData &&
                  (() => {
                    const jd = previousStepData.jobCardData as Record<
                      string,
                      unknown
                    > | null;
                    const vd = previousStepData.volumeData as Record<
                      string,
                      unknown
                    > | null;
                    ...
                    const fabricationProgress =
                      (jd?.fabricationProgress as number) || 0;
                    ...
                    let jobCards: JobCard[] = [];
                    try {
                      jobCards = vd?.jobCards
                        ? JSON.parse(vd.jobCards as string)
                        : [];
                    } catch {
                      jobCards = [];
                    }
                        ...
                        {jd && (
                          ...
                          >
                            ...
                            {completedTasks && (
                              ...
                              >
                                <strong>Công đoạn đã hoàn thành:</strong>{" "}
                                {completedTasks}
                              </div>
                            )}
                            {issues && (
                            ...
                            {jobCards.length === 0 ? (
                              ...
                              jobCards.map((card, ci) => (
                                <div
                                  key={ci}
                                  style={{
                                    border: `1px solid ${cardColors[ci % cardColors.length]}30`,
                                    borderRadius: 10,
                                    marginBottom: 10,
                                    overflow: "hidden",
                                  }}
                                  ...
                                  >
                                    <span
                                      style={{
                                        ...
                                        fontSize: "0.85rem",
                                        color:
                                          cardColors[ci % cardColors.length],
                                      }}
                                    >
                                      JC #{ci + 1}
                                    </span>
                ...
                {task.stepCode === "P5.4" &&
                  (() => {
                    ...
                    const allQty = jd
                      ? FAB_KEYS.map((k) => jd[`fab_${k}_qty`] || "").filter(
                          Boolean,
                        )
                      : [];
                    const acceptResult =
                      (formData.acceptanceResult as string) || "";
                    const resultColors: Record<
                      string,
                    ...
                    const rc = resultColors[acceptResult] || {
                      bg: "var(--bg-secondary)",
                      border: "var(--border)",
                    };
                    return (
                            ...
                            <div>
                              ...
                              <select
                                ...
                                style={{
                                  ...
                                  fontWeight: 700,
                                  borderRadius: 8,
                                  border: `2px solid ${acceptResult === "PASS" ? "#16a34a" : acceptResult === "FAIL" ? "#dc2626" : acceptResult === "CONDITIONAL" ? "#f59e0b" : "var(--border)"}`,
                                  color:
                                    acceptResult === "PASS"
                                      ? "#16a34a"
                                      : acceptResult === "FAIL"
                                        ? "#dc2626"
                                        : acceptResult === "CONDITIONAL"
                                          ? "#d97706"
                                          : undefined,
                                }}
                              >
                                ...
                                <option value="CONDITIONAL">
                                  ⚠️ CONDITIONAL — Đạt có điều kiện
                                </option>
                              </select>
                            </div>
                          </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ position: "relative", width: 280 }}>
                        <input
                          type="text"
                          className="input"
                          placeholder="🔍 Tìm kiếm mã/tên VT..."
                    ...
                    {inventoryLoading ? (
                      ...
                      (() => {
                        ...
                        if (task.stepCode === "P2.1")
                          allowedCategories = ["steel", "pipe", "valve"];
                        if (task.stepCode === "P2.2")
                          allowedCategories = ["welding", "paint"];
                        if (task.stepCode === "P2.3")
                          allowedCategories = ["bolt", "consumable"];

                        const filtered = inventoryMaterials.filter((m) => {
                          const matchCategory = allowedCategories
                        ...
                        if (filtered.length === 0)
                          return (
                            ...
                            >
                              Chưa có vật tư nào phù hợp trong kho.
                            </div>
                          );
                          ...
                          <>
                            <div
                              style={{
                                display: "grid",
                                gridTemplateColumns:
                                  "40px 100px 1.5fr 1.2fr 80px 60px 50px",
                                gap: 6,
                                padding: "8px 4px",
                                borderBottom: "2px solid var(--border)",
                                background: "var(--bg-secondary)",
                            ...
                            >
                              ...
                              {inventoryMaterials.length} vật tư tham khảo.
                            </div>
                          </>
                        );
                      })()
                  ...
                  >
                    ...
                    >
                      <h3
                        style={{
                          margin: 0,
                          fontSize: "1.1rem",
                          borderBottom: `2px solid ${task.stepCode === "P3.3" ? "#f59e0b" : "var(--accent)"}`,
                          paddingBottom: 8,
                          flex: 1,
                        }}
                      >
                        {task.stepCode === "P3.3"
                          ? "📋 Đề nghị cấp VT cho thầu phụ"
                          : task.stepCode === "P2.3"
                            ? "📦 Đề xuất vật tư"
                            : `📦 Danh sách vật tư ${task.stepCode === "P2.1" ? "(BOM)" : "(Hàn & Sơn)"}`}{" "}
                        {task.stepCode === "P2.1" ? (
                          <span
                    ...
                    <div
                      style={{
                        display: "grid",
                        gridTemplateColumns:
                          "40px 1.5fr 1fr 1fr 0.7fr 0.7fr 40px",
                        gap: 8,
                        marginBottom: 6,
                        padding: "0 4px",
                      }}
                    ...
                    {bomItems.map((item, idx) => (
                      ...
                      >
                        ...
                        <input
                          ...
                          disabled={!isActive}
                          onChange={(e) =>
                            updateBomItem(idx, "name", e.target.value)
                          }
                          style={{ fontSize: "0.85rem" }}
                        />
                        <input
                          ...
                          disabled={!isActive}
                          onChange={(e) =>
                            updateBomItem(idx, "code", e.target.value)
                          }
                          style={{
                            fontSize: "0.85rem",
                        ...
                        <input
                          ...
                          disabled={!isActive}
                          onChange={(e) =>
                            updateBomItem(idx, "spec", e.target.value)
                          }
                          style={{ fontSize: "0.85rem" }}
                        />
                        <input
                        ...
                        <select
                          ...
                          disabled={!isActive}
                          onChange={(e) =>
                            updateBomItem(idx, "unit", e.target.value)
                          }
                          style={{ fontSize: "0.85rem", padding: "3px 2px" }}
                        >
                    ...
                    >
                      Đã nhập:{" "}
                      <strong>
                        {bomItems.filter((b) => b.name.trim()).length}
                      </strong>{" "}
                      / {bomItems.length} mục
                      {task.stepCode === "P2.1" &&
                        bomItems.filter((b) => b.name.trim() && b.code.trim())
                          .length < 3 && (
                          <span style={{ color: "#dc2626", marginLeft: 10 }}>
                            ⚠️ Cần ít nhất 3 mục có tên + mã VT
                          </span>
                        )}
                    </div>
                  </div>
                ...
                {task.stepCode === "P2.4" && previousStepData?.bom && (
                  ...
                  >
                    ...
                    {previousStepData.bom.bomNotes && (
                      ...
                      >
                        <strong>Ghi chú BOM:</strong>{" "}
                        {String(previousStepData.bom.bomNotes)}
                      </div>
                    )}
                    {previousStepData.bom.bomItems &&
                    (
                      previousStepData.bom.bomItems as Array<{
                        name: string;
                        code: string;
                        spec: string;
                        quantity: string;
                      ...
                      <>
                        ...
                        <div
                          style={{
                            display: "grid",
                            gridTemplateColumns:
                              "40px 1.5fr 1fr 1fr 0.7fr 0.7fr",
                            gap: 8,
                            marginBottom: 6,
                            padding: "0 4px",
                          }}
                ...
                {/* P3.4: Production Order Items — Card Layout */}
                {task.stepCode === "P3.4" && (
                  <div
                    className="card"
                    style={{ padding: "1.5rem", marginTop: "1rem" }}
                  >
                    ...
                    >
                      Tổng:{" "}
                      <strong>
                        {woItems.filter((w) => w.content.trim()).length}
                      </strong>{" "}
                      nội dung công việc
                    </div>
                  </div>
                ...
                {task.stepCode === "P1.2A" && (
                  ...
                  >
                    ...
                    {milestones.length === 0 && (
                      ...
                      >
                        Chưa có milestone nào. Nhấn &quot;Thêm Milestone&quot;
                        để bắt đầu.
                      </div>
                    )}
                    <div
              ...
              >
                ...
                <div
                  style={{
                    display: "flex",
                    gap: 12,
                    marginBottom: showRejectForm ? 16 : 0,
                  }}
                >
                  <button
                    className="btn-accent"
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 10,
                      background: showRejectForm ? "#fef2f2" : "transparent",
                      color: "#e74c3c",
                      cursor: "pointer",
                      fontWeight: 600,
                    }}
                ...
                {showRejectForm && (
                  ...
                  >
                    ...
                    >
                      {submitting
                        ? "⏳..."
                        : "⚠️ Xác nhận từ chối → Đẩy về PM chỉnh sửa"}
                    </button>
                  </div>
                )}
              </div>
          ...
          <div>
            ...
            >
              ...
              {config.checklist
                .filter(
                  (item) =>
                    !(task.stepCode === "P5.1" && item.key.startsWith("fab_")),
                )
                .map((item) => (
                  <label
                    key={item.key}
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        isActive && handleChecklistToggle(item.key)
                      }
                      disabled={!isActive}
            ...
            {config.attachments.length > 0 && (
              ...
              >
                ...
                {config.attachments.map((att) => {
                  ...
                  const fileUrl =
                    siblingFiles && fileKey ? siblingFiles[fileKey] : null;
                  return (
                    <div
                      key={att.key}
                      style={{
                ...
                <button
                  ...
                  onClick={() => {
                    const fields = config.fields.filter(
                      (f) => f.type !== "section" && f.type !== "readonly",
                    );
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      ...
                      if (Object.keys(newData).length === 0) {
                        setError(
                          "Không tìm thấy trường dữ liệu nào khớp. Kiểm tra tên cột trong Excel.",
                        );
                        return;
                      }
                      setFormData((prev) => {
                        ...
                        // Recalculate total
                        if (
                          config.fields.some((f) => f.key === "totalEstimate")
                        ) {
                          const currencyKeys = config.fields
                            .filter((f) => f.type === "currency")
                            .map((f) => f.key);
                          next.totalEstimate = currencyKeys.reduce(
                            (sum, k) => sum + (Number(next[k]) || 0),
                            0,
                          );
                        }
                      ...
                      setSuccessMsg(
                        `✅ Đã import ${Object.keys(newData).length} trường từ Excel`,
                      );
                      setTimeout(() => setSuccessMsg(""), 3000);
                    } catch (err) {
                      setError(
                        "Lỗi khi đọc file. Vui lòng kiểm tra định dạng.",
                      );
                    }
                  }}
                />
              ...
              task.stepCode !== "P1.3" && (
                <div className="card" style={{ padding: "1.25rem" }}>
                  ...
                  {(task.stepCode !== "P4.3" ||
                      ...
                      !(() => {
                        try {
                          const items = JSON.parse(
                            (formData.qcItems as string) || "[]",
                          );
                          return items.some(
                            (q: { result: string }) => q.result === "FAIL",
                          );
                        } catch {
                          return false;
                        }
                    ...
                    (task.stepCode !== "P5.4" ||
                      !formData.acceptanceResult ||
                      formData.acceptanceResult === "PASS" ||
                      formData.acceptanceResult === "CONDITIONAL") && (
                      <button
                        className="btn-accent"
                        onClick={() => handleSubmit("complete")}
                        disabled={submitting}
                  ...
                  {rule?.rejectTo &&
                    ...
                    (task.stepCode !== "P5.4" ||
                      !formData.acceptanceResult ||
                      formData.acceptanceResult === "FAIL" ||
                      formData.acceptanceResult === "CONDITIONAL") && (
                      <div>
                        <button
                          onClick={() => setShowRejectForm(!showRejectForm)}
        ...
        <div className="card" style={{ padding: "2rem", textAlign: "center" }}>
          <p style={{ color: "var(--text-secondary)" }}>
            Bước <strong>{task.stepCode}</strong> chưa có form chi tiết. Bạn có
            thể ghi chú và hoàn thành.
          </p>
          <textarea
            value={submitNotes}
            onChange={(e) => setSubmitNotes(e.target.value)}
    ...
    case "readonly": {
      ...
      return (
        <div
          style={{
            ...baseStyle,
            color: isTotalRow ? "var(--accent)" : "var(--text-secondary)",
            fontWeight: isTotalRow ? 700 : 400,
            fontSize: isTotalRow ? "1.1rem" : undefined,
          }}
        >
          {display}
        </div>
    ...
    case "currency":
      return (
        <input
          type="text"
          inputMode="numeric"
          value={formatNumberWithCommas(value === 0 ? "0" : value || "")}
          onChange={(e) => {
...
function AssignTaskModal({
  ...
  useEffect(() => {
    apiFetch("/api/users").then((res) => {
      if (res.ok && res.users) {
        ...
        const relevant = res.users.filter((u: any) =>
          u.roleCode.startsWith(baseRole),
        );
        setUsers(relevant);
      }
      setLoading(false);
  ...
  return (
    ...
    >
      <div
        className="card w-full max-w-md bg-[var(--bg-card)] rounded-2xl p-6 shadow-2xl relative"
      ...
      >
        ...
        <p className="text-sm text-[var(--text-muted)] mb-5">
          ...
          <br />
          Vai trò phụ trách:{" "}
          <span className="font-mono bg-[var(--bg-secondary)] px-1 rounded">
            {task.assignedRole}
          </span>
        </p>
        ...
        {loading ? (
          ...
          <div className="mb-6">
            <label
              ...
              style={{ color: "var(--text-secondary)" }}
            >
              CHỌN NHÂN SỰ {task.assignedRole}
            </label>
            <select
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button
            onClick={onClose}
            className="px-4 py-2 font-semibold rounded-lg hover:opacity-80 transition"
            style={{
              background: "var(--bg-secondary)",
              color: "var(--text-primary)",
            }}
          ...
          <button
            onClick={() => {
              if (selectedUser) onSubmit(selectedUser);
            }}
