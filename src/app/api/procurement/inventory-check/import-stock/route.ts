...
// body: { rows: [{ materialCode, stock }] } — cập nhật tồn kho hiện tại (Material.currentStock) theo mã kho.
export async function POST(req: NextRequest) {
  try {
    ...
    const body = await req.json().catch(() => ({})) as { rows?: Array<{ materialCode?: unknown; stock?: unknown }> }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (r.count > 0) updated++; else notFound.push(code)
    }
    await logAudit(user.userId, 'IMPORT_STOCK', 'Material', 'bulk', { updated, notFound: notFound.length, loi: loi.length }, undefined)
    return successResponse(
      { soDongNhan: rows.length, updated, soKhongThayMa: notFound.length, notFound: notFound.slice(0, 30), loi },
      `Đã cập nhật tồn ${updated} mã${notFound.length ? `, ${notFound.length} mã không có trong danh mục` : ''}`,
    )
  } catch (err) {
