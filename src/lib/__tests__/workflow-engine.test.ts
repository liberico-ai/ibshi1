...
describe('initializeProjectWorkflow', () => {
  ...
  it('creates tasks for all non-dynamic workflow steps', async () => {
    ...
    // P5.1 is dynamic (created at runtime), so total = WORKFLOW_RULES - 1
    const DYNAMIC_STEP_COUNT = 1 // P5.1
    expect(data).toHaveLength(Object.keys(WORKFLOW_RULES).length - DYNAMIC_STEP_COUNT)
  })
  ...
  it('sets deadline for steps with deadlineDays', async () => {
    ...
    // P5.2 has no deadlineDays (P5.1 is dynamic, not in initial batch)
    const p52 = data.find(t => t.stepCode === 'P5.2')
    expect(p52?.deadline).toBeNull()
  })
})
