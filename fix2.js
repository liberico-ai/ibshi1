...
const parts = txt.split(hookEndStr);
if (parts.length > 1) {
  txt = parts[0] + hookEndStr + "\n" + wbsFuncCode + "\n" + parts[1];
}

// Replace P1.2A
const p12aRegex = /\{\/\* P1\.2A: Dynamic WBS Table — based on BCTH-IBSHI-QLDA-095 \*\/\}[\s\S]*?\}\(\)\)}/;
txt = txt.replace(p12aRegex, "{/* P1.2A: Dynamic WBS Table */}\n{task.stepCode === 'P1.2A' && renderWbsTableUI(isActive, formData['wbsItems'])}");

// Replace P3.1/P1.3 read-only table
const legacyWbsTableRegex = /<div className="card"[^>]*>[\s\S]*?Bảng kế hoạch tổng thể triển khai[\s\S]*?<\/table>[\s\S]*?<\/div>\s*<\/div>/;
txt = txt.replace(legacyWbsTableRegex, "<div style={{ gridColumn: '1 / -1', width: '100%' }}>{renderWbsTableUI(false, (previousStepData?.plan as any)?.wbsItems || [])}</div>");

// Fix compiler errors in TSX comparisons
txt = txt.replaceAll("{(task.stepCode === 'P2.4') &&", "{(task.stepCode === 'P2.4' || task.stepCode === 'P1.2A') &&");
