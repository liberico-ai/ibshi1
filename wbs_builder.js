...
try {
  ...
  const renderWbsTableUI = (isWbsEditable: boolean, wbsItemsData: any) => {
    ...
    const save = (next: WbsRow[]) => handleFieldChange('wbsItems', JSON.stringify(next));
...
`;
  ...
  const hookEndStr = "const isAnyStepDisabled = false;";
  const hookEndIdx = content.indexOf(hookEndStr);
  if (hookEndIdx === -1) {
    console.error("Could not find hookEndStr!");
    process.exit(1);
  }
  content = content.substring(0, hookEndIdx + hookEndStr.length) + "\n\n" + wbsFuncCode + "\n" + content.substring(hookEndIdx + hookEndStr.length);

  // Remove the old P1.2A inline code
  const p12Start = content.indexOf("{/* P1.2A: Dynamic WBS Table — based on BCTH-IBSHI-QLDA-095 */}");
  const p12End = content.indexOf("{/* P2.4 dynamic cost adjustment forms */}");
  if (p12Start !== -1 && p12End !== -1) {
    const endParentheses = content.indexOf("})()}", p12Start) + 5;
    content = content.substring(0, p12Start) + 
      "{/* P1.2A: Dynamic WBS Table UI */}\n" +
      "{(task.stepCode === 'P1.2A') && renderWbsTableUI(isActive, formData['wbsItems'])}\n\n" +
      content.substring(endParentheses);
  }

  // Remove the legacy P1.3 WBS table
  const p13TitleIdx = content.indexOf("📋 WBS đã duyệt (P1.2A)</h3>");
  if (p13TitleIdx !== -1) {
    const p13CardStart = content.lastIndexOf("<div className=\"card\"", p13TitleIdx);
    const tblEndIdx = content.indexOf("</table>", p13CardStart);
    const div1End = content.indexOf("</div>", tblEndIdx) + 6;
    const div2End = content.indexOf("</div>", div1End) + 6;
    const div3End = content.indexOf("</div>", div2End) + 6; // To close the card completely
    
    // Validate we got the whole card
    content = content.substring(0, p13CardStart) + 
      "            {/* P1.3: Unified WBS Table UI */}\n" +
      "            <div style={{ width: '100%', marginTop: '1rem' }}>\n" +
      "              {renderWbsTableUI(false, (previousStepData?.plan as any)?.wbsItems)}\n" +
      "            </div>\n" +
      content.substring(div3End);
  }

  // Fix P2.4 TS errors
  content = content.replaceAll("{(task.stepCode === 'P2.4') && renderEstTable(", "{(task.stepCode === 'P2.4' || task.stepCode === 'P1.2A') && renderEstTable(");
